...
class AMFBaseDeserializer {
    ...
    function mapClass($typeIdentifier)
    {
		...
		$include = FALSE;
		$exp = explode('/',$mappedClass);
		$nbEl = count($exp)-1;
		$exp[$nbEl] = "class.".$exp[$nbEl].".php";
		$mappedClass2 = implode('/',$exp);
		$type = "old";
		if(file_exists($_SERVER['DOCUMENT_ROOT'].'/classesBT/' . $mappedClass2))
		{
			$include = $_SERVER['DOCUMENT_ROOT'].'/classesBT/' . $mappedClass2;
			$type = "bt";
		}
		...
		if($include !== FALSE)
		{
			...
			if($type == "bt"){
				$classname = str_replace(array('class.','.php'),'',$exp[$nbEl]);
			}else{
