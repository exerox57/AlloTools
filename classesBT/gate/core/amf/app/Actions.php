...
function executionAction (&$amfbody) 
{
	...
	if (!$amfbody->isSpecialHandling() || $amfbody->isSpecialHandling(array('describeService', 'pageFetch', 'RemotingMessage')))
	{
		...
		else if($specialHandling == 'pageFetch')
		{
			...
			if(isset($construct->sessionValide) && $construct->sessionValide==false){
				$results['sessionExpiree']=true;
			}
			else $results = Executive::doMethodCall($amfbody, $construct, $method, $args); // do the magic
			global $amfphp;
