...
class Gateway {
	...
	function service() {
		...
		if(isset($GLOBALS["HTTP_RAW_POST_DATA"]) && $GLOBALS["HTTP_RAW_POST_DATA"] != "")
		{
			...
			/////////////////////////////////////
			/////   RAJOUT BATITRADE LOG DE LA COMMANDE TODO
			/////////////////////////////////////
			$a = $amf->getBodyAt(0);
			if($a != null){
				
				$class = $a->className;
				$methode = $a->methodName;
				try {
					$value = json_encode($a->getValue());
				}
				catch (Exception $e) {
					$value = "NaN convertit en 0 : ".json_encode(str_replace('NaN', '0', $a->getValue()));
				}
				if ($methode == "assistantCommand" && strpos($value, 'bugReport') !== false) $value = 'bugReport'; // Permet de ne pas envoyer l'image en base 64.
				$mess =  "[".date("d-m-Y h:m:s")."] service:$class methode:$methode valeur:$value";
				if(!defined("CURRENT_ACTION_AMF")){
					define("CURRENT_ACTION_AMF","methode:$methode+valeur:$value");
				}
				
				$fileStream = fopen($_SERVER["DOCUMENT_ROOT"]."/logs/logCmdAMF.txt","a");
				fwrite($fileStream,$mess."\n");
				fclose($fileStream);
				
				$r = $a->getResults();
				if($r instanceof ErrorMessage){
					$mess = "[".date("d-m-Y h:m:s")."] ".$r->faultDetail." ".$r->faultString;
					$fileStream = fopen($_SERVER["DOCUMENT_ROOT"]."/logs/logData.txt","a");
					fwrite($fileStream,$mess."\n");
					fclose($fileStream);	
				}
			}
			////////////////////////////////////////
			/////   FIN RAJOUT BATITRADE LOG DE LA COMMANDE TODO
			////////////////////////////////////////
